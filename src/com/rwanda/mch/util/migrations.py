...
def migrate_simcards():
  ...
  ans_chws = fetch_data(MCONN, QRY_CHWS)
  ans_ambs = fetch_data(MCONN, QRY_AMBS)
  ans_dtms = fetch_data(MCONN, QRY_DTMS)
  ans_stfs = fetch_data(MCONN, QRY_STFS)
  ans_hdis = fetch_data(MCONN, QRY_HDIS)
  ans_mnes = fetch_data(MCONN, QRY_MNES)
  ans_sups = fetch_data(MCONN, QRY_SUPS)
...
def migrate_field(report, key, max_value, min_value, default):
  ...
  ans = fetch_data(MCONN, QRY % {'key': key, 'repid': report.id})
  value = default
  for an in ans:
    value = an.key
    if an.value:
...
def migrate_smsfields():
  from underscore import _
  from sms.api.messaging.models import SMSReport, SMSReportField
  from util.mch_util import load, json
  smsfs = []
  try:
    rpts = SMSReport.objects.filter(module_pk = 1).order_by('id')
    data = json.loads(load('db/smsfield.json'))
    i = 1
    
    for rpt in rpts:
     ans = [d for d in data if d['keyword'] == rpt.keyword]
     oans = _(ans).chain().sortBy('position_after_sms_keyword').value()
      
     for p in oans:
      tmp_p = None
      endresource_pk = rpt.id
      depnd_pk = None
      if p.get('depends_on_value_of_id'):
        depnd = _(oans).chain().filter(lambda x: x['id'] == p['depends_on_value_of_id']).value()
        deprecord = fetch_smsfield(depnd[0]['key'], endresource_pk)
        if deprecord: depnd_pk = getattr(deprecord, 'indexcol')
        if not depnd_pk:
          tmp_p = depnd[0]
      if tmp_p:
        migrate_field(tmp_p, i,  depnd_pk, endresource_pk)
        i += 1
      fields = migrate_field(p, i,  depnd_pk, endresource_pk)
      smsfs.append(fields)
      i += 1    
  except Exception, e:
    print e
...
def migrate_field(p, indexcol,  depnd_pk, endresource_pk):
  f = {}
  try:
    if p['key'] == 'cl': p['key'] = 'hc'
    fields = {
        #'indexcol': indexcol,
        ...
        'name_rw' : p['title_rw'],
        'category_rw' : p['category_rw'],
        'prefix' : p['prefix'],
        'description' : p['description'],
        'type_of_value' : p['type_of_value'],
        'upper_case' : p['upper_case'],
        'lower_case' : p['lower_case'],
        'minimum_value' : p['minimum_value'],
        'maximum_value' : p['maximum_value'],
        'minimum_length' : p['minimum_length'],
        'maximum_length' : p['maximum_length'],
        'position_after_sms_keyword' : p['position_after_sms_keyword'],
        'depends_on_value_of_pk' : depnd_pk,
        'dependency' : p['dependency'],
        'allowed_value_list' : p['allowed_value_list'],
        'only_allow_one' : p['only_allow_one'],
        'required' : p['required'], 
        'sms_report_pk' : endresource_pk
       }
    fs = fetch_smsfield(fields['code'], fields['sms_report_pk'])
    if fs: fields['indexcol'] = fs.indexcol
    f = fields
    p = migrate(table='smsfield', fields = f)
  except Exception, e:
    print e
  return f 

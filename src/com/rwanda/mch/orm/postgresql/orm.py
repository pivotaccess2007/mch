...
'''
  ...
  @property
  def subs(self):
    return self.kwargs.get('subs', {})

  @property
  def joins(self):
    return self.kwargs.get('joins', {})

  @property
  def group_by(self):
    return self.kwargs.get('group_by', [])
  ...
  def query(self):
    ...
    sncs  = ORM.fetch_columns(self.tablenm)
    cols = cols.split(",")
    
    for col in cols:
      index = cols.index(col)
      if col.find('*') > -1: col = col.replace('*', '%s.*' % self.tablenm)
      else: col = '%s.%s' % (self.tablenm, col)
      cols[index] = col
    cols = u', '.join(cols)
    
    for col in sncs:
        #print "QRY: ", qry
        if qry.find(col) > -1  and qry.find('_%s'%col) < 0 and qry.find('%s.%s' % (self.tablenm, col)) < 0 and re.search(r'\b%s\b'%col, qry):
          qry = qry.replace(col, '%s.%s' % (self.tablenm, col) )
    ...
    for n in joins:
      ...
      q1 = ''
      if type(k) == tuple:
        alias = alias = '%s_%s' % (k[1], k[0])
        k = k[0]
      clause = '%s.%s = %s.indexcol' % (self.tablenm, c, q)
      if q == self.tablenm:
        clause = '%s.%s = %s1.indexcol' % (self.tablenm, c, q)
        q1 = q
        q = '%s %s1' % (q, q)
    
      annot.append(u'( %s.%s ) AS %s' % (q1 if q1 else q, k, alias))
      if q not in joins_tbls:
        joins_tbls.add(q)         
        rez = u' %s %s ' % (rez , u'LEFT JOIN %s ON ( %s )' % ('%s1'%q1 if q1 else q, clause) )
    ...
    for s in subs:
      ...
      if type(k) == tuple:
        alias = alias = '%s_%s' % (k[1], k[0])
        k = k[0]
      clause = '%s.indexcol = %s.%s' % (q, self.tablenm, c)
      if q == self.tablenm:
        clause = '%s1.indexcol = %s.%s' % (q, self.tablenm, c)
        q1 = q
        q = '%s %s1' % (q, q)    
      annot.append(u'(SELECT %s.%s FROM %s WHERE %s) AS %s' % ('%s1'%q1 if q1 else q, k, q, clause, alias))
    ...
    for k in parts:
      ...
      if type(it) ==  type(('query', 'conds')):
        ...
        it  = it[0]
      #print "V: ", it, self.tablenm, q, k
      if it.find('*') > -1: it = it.replace('*', '%s.*' % self.tablenm)
      for col in sncs:
        #print "Q: ", q
        if q.find(col) > -1 and q.find('_%s'%col) < 0 and q.find('%s.%s' % (self.tablenm, col)) < 0  and re.search(r'\b%s\b'%col, q):
          q = q.replace(col, '%s.%s' % (self.tablenm, col) ) 
      annot.append(u'(SELECT %s FROM %s%s) AS %s' % (it, self.tablenm, q, k))
      # annot.append(u'(SELECT %s %s) AS %s' % (it, q, k))
    annot.append(u'%s FROM %s%s%s' % (cols, self.tablenm, rez, qry))
    qry = u', '.join(annot)
    if group_by:  qry = u' '.join([ qry, u' GROUP BY (%s)' % ( ', '.join(['%s.%s' % (self.tablenm, x) for x in group_by])) ])
    #print "QUERY: ", qry
...
'''
    ...
    for col in cols:
      ...
      try:  vals.append(elval.decode('utf-8'))
      except UnicodeDecodeError:  vals.append(elval.decode('latin-1'))
    if not ans:
      ...
      for k in dat:
        ...
        try:
          ...
          elval = curz.mogrify(u'%s', (dval, ))
        try:  dem.append(u'%s = %s' % (k, elval.decode('utf-8')))
        except UnicodeDecodeError:  dem.append(u'%s = %s' % (k, elval.decode('latin-1')))
  ...
  @classmethod
  def fetch_columns(self, tbl):
    '''Creates the table `tbl`, if necessary.'''
    try:
      curz  = self.postgres.cursor()
      curz.execute(u'SELECT column_name FROM information_schema.columns WHERE table_name = %s', (tbl,))
      ans = []
      for an in curz.fetchall(): ans.append(an[0])
      self.postgres.commit()
      return ans
    except Exception, e:
      self.postgres.rollback()
      self.postgres.commit()
      raise Exception, ('No columns: ' + str(e))

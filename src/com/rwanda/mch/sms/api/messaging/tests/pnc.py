...
('PNC' , 'PNC 0788660270010918 PNC2 OYA 04.09.2018 VO PC OE NS MA JA FP FE DS DI SA RB HY CH PR MW'),
...
rectifier.validate_unique_codes(position = 2, key = 'pnc_visit', errcodes = ["missing_pnc_visit", "invalid_pnc_visit"] )
rectifier.validate_unique_codes(position = 3, key = 'pnc1', errcodes = ["missing_nbc1", "invalid_pnc1"] )
rectifier.validate_delivery_date(position = 4)
rectifier.validate_current_symptoms(position = 5)
rectifier.validate_unique_codes(position = 6, key = 'intervention', errcodes = ['missing_intervention', 'invalid_intervention'])
rectifier.validate_unique_codes(position = 7, key = 'mother_status', errcodes = ['missing_mother_status', 'invalid_mother_status'])
rectifier.invalid_sms_codes()
rectifier.validate_duplicate_report()

print rectifier.errors

pnc = Persister(rectifier).save()

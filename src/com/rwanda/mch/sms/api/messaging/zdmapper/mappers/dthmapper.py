...
class Deathmapper(object):
    """ Deathmapper map the sms report appropriately to db columns structure """

    def __init__(self, report):
        self.dth = Death(report.nid, pregnancy_pk = report.pregnancy_pk, birth_date = None, child_number = None)
    ...
    def get_fields(self):
        ...
        try:
            ...
            self.FIELDS.update( {
                                  ...
                                  'death_code': self.code(self.death),
    ...
    def store(self):
        try:    
            dth  =   self.dth.save(self.orm, self.get_fields())
            return dth             
        except Exception, e:
            print "STORE DTH: %s" % e
            pass
